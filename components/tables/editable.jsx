...
function EditorToolbar({ addRow, discardChanges, hasUnsavedRows, saveChanges }) {
  ...
  return (
    // Contenedor de la barra de herramientas con estilos personalizados
...
export function EditableDataGrid({ columns, data, saveAction }) {
  ...
  const saveChanges = useCallback(() => {
    ...
    if (Object.keys(invalidCells).length > 0) {
      ...
      return { error: "Hay celdas con valores inválidos" } // Evitar el guardado si hay errores
    }

    // Si no hay celdas inválidas, proceder con el guardado
    setInvalidCells({}) // Limpiar celdas inválidas
    saveAction(unsavedRows, newRows) // Ejecutar la acción de guardado
  }, [saveAction])
  ...
  const addRow = () => {
    const newRowId = Math.max(rowNum + 1, apiRef.current.getRowsCount() + 1) // Generar IDs autoincrementales no duplicados (error que ocurría al borrar y añadir filas nuevas continuamente)
    // newRowId no se almacena en la base de datos, el ID se usa solo en el DataGrid
  ...
  // mmmmm -0queso
  const tableColumns = [gridActionColumn, ...columns] // Añadir las acciones como primera columna de la tabla
  ...
  const getRowClassName = useCallback(({ id }) => {
    ...
    if (unsavedRow) {
      // Si la fila ha sido modificada, se comprueba qué cambio se ha hecho
      if (unsavedRow._action === "delete") {
        // Si el cambio implica su eliminación, se etiqueta de esa forma
        return "row--removed";
      }
      // Sino se etiqueta como modificada
      return "row--edited";
    } else if (unsavedChangesRef.current.newRows[id]) {
      // Si la fila es nueva, se etiqueta de esa forma
      return "row--added"
    }
    return "" // Devuelve una string vacía (la clase) si la fila existía de antes y no se ha modificado
  }, [])
  ...
  const handleCellClick = useCallback((params, event) => {
    ...
    if (!params.isEditable || !event.currentTarget.contains(event.target)) {
      return;
    }
